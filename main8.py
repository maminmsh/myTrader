...
def calculate_rsi(df, period=14):
    ...
    loss = loss.replace(0, 1e-10)  # جلوگیری از تقسیم بر صفر
    rs = gain / loss
    df['RSI'] = 100 - (100 / (1 + rs))
    return df
...
def generate_signals(df):
    ...
    df['Buy_Signal'] = (df['RSI'] < 40) & (df['MACD'] > df['Signal']) & (df['close'] < df['Bollinger_Lower'])
    df['Sell_Signal'] = (df['RSI'] > 60) & (df['MACD'] < df['Signal']) & (df['close'] > df['Bollinger_Upper'])
    return df
...
# رسم نمودار باندهای بولینگر
def plot_bollinger_bands(df):
    plt.figure(figsize=(14, 7))
    plt.plot(df['time'], df['close'], label='Close Price', color='blue')
    plt.plot(df['time'], df['Bollinger_Upper'], label='Bollinger Upper', color='red')
    plt.plot(df['time'], df['Bollinger_Lower'], label='Bollinger Lower', color='green')
    plt.fill_between(df['time'], df['Bollinger_Lower'], df['Bollinger_Upper'], color='gray', alpha=0.1)
...
def main():
    ...
    plot_bollinger_bands(df)  # نمودار باندهای بولینگر
